...
// Ensure path module is available (already imported at top)
// const path = require('path');


const tempDir = process.env.TEMP_DIR_INTERNAL || path.join(__dirname, '../temp');
...
const executeDocker = (language, jobDir, filename, input) => {
    return new Promise((resolve, reject) => {
        ...
        // For sibling containers, we need the HOST path, not the internal container path.
        // We calculate the relative path from the internal temp dir, then append to host temp dir.
        const relativeJobPath = path.relative(tempDir, jobDir);
        const mountPath = process.env.TEMP_DIR_HOST
            ? path.join(process.env.TEMP_DIR_HOST, relativeJobPath)
            : jobDir; // Fallback to internal path if no host path provided (dev mode/non-docker)
        ...
        console.log(`[Worker] Preparing to run ${language} for job in ${jobDir} (Mounting: ${mountPath})`);
